      ...
      if (currentActive) {
        if (bytesExchanged > highThreshold || bytesExchanged < lowThreshold) {
          this.addCustomEvent(
            new Date().toJSON(),
            "quality",
            bytesExchanged > highThreshold ? "peak" : "drop",
            `A variation has been detected for the ${property.includes("out") ? "outbound" : "inbound"} ${data.type} steam. Could be linked to a ${bytesExchanged > highThreshold ? "unmute" : "mute"}`,
            {
              direction: property.includes("out") ? "outbound" : "inbound",
              kind: data.type,
              ssrc: data.ssrc,
